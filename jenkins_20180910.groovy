pipeline {
    ...
    parameters {
        ...
        string(name: 'Volsize', defaultValue: '50', description: 'Repo url')
            ...
            steps{               
                script{
                    ...
                    withEnv(lines_variables){
                        sh 'env'
                        vzAWS_CF_CICD_V2 awsenv: 'NONPROD', parameter: '', playbook: '', region: '$AWS_REGION', role: 'App', stackName: '$StackName', stashBranch: 'ecs', stashUrl: 'ssh://onestash.verizon.com:7999/ejavb/ejav.git', templateName: 'cloudformation/vzw_ejav_ecs.json', templateParameter: '{"Volsize":"${Volsize}","Environment":"${ENVIRONMENT}","InstanceType":"${TYPE}","AMI":"${AMIID}","AWSAccount":"VZW","SUBNET":"${SUBNET}","AWSAccount":"VZW","Role":"App","AppID":"EJAV","SecurityGroupIds":"${SECGROUPS}"}'
                    }
                }
            }
        }
